...
fn test_authority_module() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// );
			// assert_eq!(
			// 	Currencies::free_balance(CurrencyId::Token(TokenSymbol::AISD), &AccountId::from(BOB)),
			// 	amount(500)
			// );
            //
			// // delay < SevenDays
...
fn test_evm_module() {
	ExtBuilder::default()
		.balances(vec![
			(alice_account_id(), CurrencyId::Token(TokenSymbol::AISC), amount(1 * MILLI_AISC)),
			(bob_account_id(), CurrencyId::Token(TokenSymbol::AISC), amount(1 * MILLI_AISC)),
		])
		.build()
		.execute_with(|| {
			assert_eq!(Balances::free_balance(alice_account_id()), amount(1 * MILLI_AISC));
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_AISC));
			...
			assert_ok!(Currencies::transfer(
				...
				CurrencyId::Token(TokenSymbol::AISC),
				amount(10 * MICRO_AISC)
			));
			assert_eq!(Balances::free_balance(alice_account_id()), 1009999999999989633000000000000000);
			assert_eq!(Balances::free_balance(bob_account_id()), amount(1 * MILLI_AISC) - amount(10 * MICRO_AISC));
		});
}
