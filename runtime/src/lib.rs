...
pub use frame_support::{
    ...
    traits::{
        schedule::Priority, EnsureOrigin, KeyOwnerProofSystem, OriginTrait, Randomness,
...
pub use runtime_common::{
    BlockLength, BlockWeights, GasToWeight, OffchainSolutionWeightLimit, Price, Rate, Ratio,
    ...
    fn check_dispatch_from(&self, origin: Origin) -> DispatchResult {
        ensure_root(origin.clone()).or_else(|_| match self {
            AuthoritysOriginId::Root => <EnsureDelayed<
                ...
                BlockNumber,
                OriginCaller,
            > as EnsureOrigin<Origin>>::ensure_origin(
                origin
            )
            .map_or_else(|_| Err(BadOrigin.into()), |_| Ok(())),
        })
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("aisland"),
    impl_name: create_runtime_str!("aisland"),
    authoring_version: 1,
    spec_version: 5,
    impl_version: 5,
...
parameter_types! {
    pub const BasicDeposit: Balance =      100 * AISC;
    pub const FieldDeposit: Balance =        1 * AISC;
    pub const SubAccountDeposit: Balance =  20 * AISC;
...
parameter_types! {
    pub const IndexDeposit: Balance = 1 * AISC;
}
...
parameter_types! {
    pub const GetNativeCurrencyId: CurrencyId = CurrencyId::Token(TokenSymbol::AISC);
    pub const GetStableCurrencyId: CurrencyId = CurrencyId::Token(TokenSymbol::AISD);
...
parameter_types! {
    pub const TransactionByteFee: Balance = 10 * MILLI_AISC;
...
parameter_types! {
    ...
    // 10 AISC minimum storage deposit
    pub const NewContractExtraBytes: u32 = 10_000;
    pub const StorageDepositPerByte: Balance = 1 * MILLI_AISC;
    pub const MaxCodeSize: u32 = 60 * 1024;
    pub NetworkContractSource: H160 = H160::from_low_u64_be(0);
    pub const DeveloperDeposit: Balance = 1_000 * AISC;
    pub const DeploymentFee: Balance    = 100 * AISC;
}
...
parameter_types! {
    // note: if we add other native tokens (AISD) we have to set native
    // existential deposit to 0 or check for other tokens on account pruning
    pub const NativeTokenExistentialDeposit: Balance =       1 * AISC;
    pub const MaxNativeTokenExistentialDeposit: Balance = 1000 * AISC;
...
parameter_types! {
    ...
    pub const CandidacyDeposit: Balance =   1_000_000 * primitives::currency::AISC;
    pub const MinLockAmount: Balance =        100_000 * primitives::currency::AISC;
    pub const TotalLockedCap: Balance = 2_000_000_000 * primitives::currency::AISC;
}
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic
    {
        ...
        // Nft Pallet
        Nft: orml_nft::{Module, Call, Storage, Config<T>}= 60,
        // !INK Smart Contracts
